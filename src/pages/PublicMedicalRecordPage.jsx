  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6" dir="rtl">
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        {recent_lab_tests.length > 0 && <Section title="التحاليل المخبرية" icon={Activity}><div className="space-y-2">{recent_lab_tests.map(item => <div key={item.id || item.test_name} className="flex justify-between gap-3 rounded-xl bg-gray-50 p-3 text-sm"><span className="font-medium">{item.test_name || item.name}</span><span className="text-gray-600">{item.result || item.status || '—'}</span></div>)}</div></Section>}
        ...
        {vaccinations.length > 0 && <Section title="التطعيمات" icon={Syringe}><div className="flex flex-wrap gap-2">{vaccinations.map(item => <span key={item.id || item.name} className="rounded-full bg-amber-50 px-3 py-1.5 text-sm text-amber-800">{item.name || item.vaccine_name}</span>)}</div></Section>}
