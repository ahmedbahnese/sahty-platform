...
import {
  ...
  Syringe, UserRound, X, ArrowUpRight, Clock3, ShieldCheck, ScanLine, Printer,
} from 'lucide-react'
...
function PatientHeader({ patient, onClose, onPrint }) {
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-l from-[#063b70] via-[#087f9b] to-[#12b8c6] px-4 py-2 text-white"><div className="flex items-center gap-2 text-sm"><span className="font-bold">ملف المريض</span><span className="text-cyan-100">/</span><span>لوحة الطبيب</span></div><div className="flex gap-2"><button className="rounded-lg p-2 hover:bg-white/10" title="طباعة التقرير الطبي" onClick={onPrint}><Printer className="h-4 w-4" /></button><button className="rounded-lg p-2 hover:bg-white/10" title="إغلاق الملف" onClick={onClose}><X className="h-4 w-4" /></button></div></div>
...
export default function DoctorPatientsPage() {
  ...
  const [vitalsForm, setVitalsForm] = useState({ blood_pressure: '', pulse: '', temperature: '', respiratory_rate: '', oxygen_saturation: '', weight: '', notes: '' })
  ...
  const submitVitals = async event => {
    event.preventDefault(); if (!selected) return
    const vital_signs = Object.fromEntries(Object.entries(vitalsForm).filter(([key, value]) => key !== 'notes' && value !== ''))
    if (!Object.keys(vital_signs).length) { setMessage('أدخل قراءة واحدة على الأقل للعلامات الحيوية'); return }
    setVitalsBusy(true)
    try {
      const response = await fetch(`/api/doctors/me/patients/${selected.id}/medical-records`, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: vitalsForm.notes || 'تسجيل العلامات الحيوية', vital_signs }) })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'تعذر حفظ العلامات الحيوية')
      setRecord(previous => ({ ...previous, medical_records: [data.record, ...(previous.medical_records || [])] }))
      setVitalsForm({ blood_pressure: '', pulse: '', temperature: '', respiratory_rate: '', oxygen_saturation: '', weight: '', notes: '' })
      setMessage('تم حفظ قراءة العلامات الحيوية في التاريخ السريري')
    } catch (error) { setMessage(error.message) } finally { setVitalsBusy(false) }
  }
  ...
  const renderTab = () => {
    ...
    const vitalHistory = (record.medical_records || []).filter(item => item.vital_signs && Object.keys(item.vital_signs).length)
    const vitals = vitalHistory[0]?.vital_signs || {}
    ...
    if (activeTab === 'vitals') return <div className="space-y-4"><div className="rounded-xl border border-rose-100 bg-rose-50/50 p-4"><h3 className="mb-4 flex items-center gap-2 font-bold text-slate-900"><HeartPulse className="h-5 w-5 text-rose-600" />العلامات الحيوية وتاريخ ضغط الدم والنبض</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['ضغط الدم', vitals.blood_pressure || vitals.bloodPressure || 'غير مسجل'], ['النبض', vitals.pulse || vitals.heart_rate || 'غير مسجل'], ['الحرارة', vitals.temperature || 'غير مسجل'], ['تشبع الأكسجين', vitals.oxygen_saturation || vitals.spo2 || 'غير مسجل']].map(([label, value]) => <Metric key={label} label={label} value={value} icon={HeartPulse} tone="rose" />)}</div></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-4"><h3 className="mb-3 font-bold text-slate-800">السجل التاريخي للقراءات</h3>{vitalHistory.length ? <div className="overflow-x-auto"><table className="w-full min-w-[560px] text-right text-sm"><thead><tr className="border-b border-slate-200 text-xs text-slate-500"><th className="p-2">التاريخ</th><th className="p-2">ضغط الدم</th><th className="p-2">النبض</th><th className="p-2">الحرارة</th><th className="p-2">ملاحظات</th></tr></thead><tbody>{vitalHistory.map(item => <tr key={item.id} className="border-b border-slate-100"><td className="p-2">{formatDate(item.visit_date)}</td><td className="p-2 font-semibold text-rose-700">{display(item.vital_signs.blood_pressure || item.vital_signs.bloodPressure)}</td><td className="p-2 font-semibold text-blue-700">{display(item.vital_signs.pulse || item.vital_signs.heart_rate)}</td><td className="p-2">{display(item.vital_signs.temperature)}</td><td className="p-2 text-slate-600">{display(item.notes)}</td></tr>)}</tbody></table></div> : <p className="rounded-xl bg-white p-6 text-center text-sm text-slate-500">لا توجد قراءات تاريخية مسجلة.</p>}</div><div className="rounded-xl border border-teal-100 bg-teal-50/60 p-4"><h3 className="mb-3 flex items-center gap-2 font-bold text-teal-950"><Plus className="h-4 w-4" />إضافة قراءة جديدة</h3><form onSubmit={submitVitals} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Input value={vitalsForm.blood_pressure} onChange={e => setVitalsForm(f => ({ ...f, blood_pressure: e.target.value }))} placeholder="ضغط الدم مثل 120/80" aria-label="ضغط الدم" /><Input value={vitalsForm.pulse} onChange={e => setVitalsForm(f => ({ ...f, pulse: e.target.value }))} placeholder="النبض / دقيقة" aria-label="النبض" type="number" min="0" /><Input value={vitalsForm.temperature} onChange={e => setVitalsForm(f => ({ ...f, temperature: e.target.value }))} placeholder="الحرارة °C" aria-label="الحرارة" /><Input value={vitalsForm.oxygen_saturation} onChange={e => setVitalsForm(f => ({ ...f, oxygen_saturation: e.target.value }))} placeholder="تشبع الأكسجين %" aria-label="تشبع الأكسجين" /><Input value={vitalsForm.respiratory_rate} onChange={e => setVitalsForm(f => ({ ...f, respiratory_rate: e.target.value }))} placeholder="معدل التنفس" aria-label="معدل التنفس" /><Input value={vitalsForm.weight} onChange={e => setVitalsForm(f => ({ ...f, weight: e.target.value }))} placeholder="الوزن كجم" aria-label="الوزن" /><Input value={vitalsForm.notes} onChange={e => setVitalsForm(f => ({ ...f, notes: e.target.value }))} placeholder="ملاحظات القراءة" aria-label="ملاحظات القراءة" className="sm:col-span-2" /><Button type="submit" disabled={vitalsBusy} className="sm:col-span-2 lg:col-span-4">{vitalsBusy ? 'جارٍ الحفظ...' : 'حفظ القراءة في التاريخ السريري'}</Button></form></div></div>
  ...
  return <><style>{`@media print { @page { size: A4 landscape; margin: 12mm; } body * { visibility: hidden !important; } [data-print-area], [data-print-area] * { visibility: visible !important; } [data-print-area] { position: absolute; inset: 0; width: 100%; background: #fff; padding: 0; } [data-print-area] button, [data-print-area] nav { display: none !important; } }`}</style><div className="space-y-5">
